...
#include "llpreprocessor.h"		// For LL_INLINE

// This is a personal speed optimization of phmap containers that removes the
// fiddling with the key hashes: those shall be good enough already (the hash
// functions in use by the viewer code have been optimized for speed and
// efficiency, so no need to complexify the key hash generation and slow it
// down for no practical benefit). HB
#define PHMAP_NO_MIXING

// Define to 1 if you wish to allow boost shared mutexes use. Not enabled here,
// since anyway, we use C++17 to compile the viewer and phmap can therefore use
// the standard C++17 shared mutexes... HB
#define PHMAP_USE_BOOST_MUTEX 0
...
// --------------------------------------------------------------------------
LL_INLINE size_t HashSeed(const ctrl_t* ctrl) {
...
LL_INLINE size_t H1(size_t hashval, const ctrl_t* ctrl) {
...
LL_INLINE size_t H1(size_t hashval, const ctrl_t* ) {
    return (hashval >> 7);
}
...
LL_INLINE ctrl_t H2(size_t hashval)       { return (ctrl_t)(hashval & 0x7F); }

LL_INLINE bool IsEmpty(ctrl_t c)          { return c == kEmpty; }
LL_INLINE bool IsFull(ctrl_t c)           { return c >= static_cast<ctrl_t>(0); }
LL_INLINE bool IsDeleted(ctrl_t c)        { return c == kDeleted; }
LL_INLINE bool IsEmptyOrDeleted(ctrl_t c) { return c < kSentinel; }
...
// --------------------------------------------------------------------------
LL_INLINE __m128i _mm_cmpgt_epi8_fixed(__m128i a, __m128i b) {
...
LL_INLINE bool IsValidCapacity(size_t n) { return ((n + 1) & n) == 0 && n > 0; }
...
//   FULL -> DELETED
// --------------------------------------------------------------------------
LL_INLINE void ConvertDeletedToEmptyAndFullToDeleted(
...
// --------------------------------------------------------------------------
LL_INLINE size_t NormalizeCapacity(size_t n)
{
...
// --------------------------------------------------------------------------
LL_INLINE size_t CapacityToGrowth(size_t capacity)
{
    assert(IsValidCapacity(capacity));
    // `capacity*7/8`
...
// --------------------------------------------------------------------------
LL_INLINE size_t GrowthToLowerboundCapacity(size_t growth)
{
    // `growth*8/7`
    PHMAP_IF_CONSTEXPR (Group::kWidth == 8) {
        if (growth == 7)
...
LL_INLINE void RecordRehashSlow(HashtablezInfo*, size_t ) {}

static LL_INLINE void RecordInsertSlow(HashtablezInfo* , size_t, size_t ) {}

static LL_INLINE void RecordEraseSlow(HashtablezInfo*) {}

static LL_INLINE HashtablezInfo* SampleSlow(int64_t*) { return nullptr; }
static LL_INLINE void UnsampleSlow(HashtablezInfo* ) {}

class HashtablezInfoHandle
{
public:
    LL_INLINE void RecordStorageChanged(size_t , size_t ) {}
    LL_INLINE void RecordRehash(size_t ) {}
    LL_INLINE void RecordInsert(size_t , size_t ) {}
    LL_INLINE void RecordErase() {}
    friend LL_INLINE void swap(HashtablezInfoHandle& ,
                            HashtablezInfoHandle& ) noexcept {}
};

static LL_INLINE HashtablezInfoHandle Sample() { return HashtablezInfoHandle(); }
...
static LL_INLINE void SetHashtablezEnabled(bool ) {}
static LL_INLINE void SetHashtablezSampleParameter(int32_t ) {}
static LL_INLINE void SetHashtablezMaxSamples(int32_t ) {}
...
public:
    ...
    LL_INLINE void _erase(const_iterator cit) { _erase(cit.inner_); }
    ...
    template <class K = key_type>
    LL_INLINE iterator find(const key_arg<K>& key) {
        return find(key, this->hash(key));
    }

    template <class K = key_type>
    LL_INLINE const_iterator find(const key_arg<K>& key, size_t hashval) const {
        return const_cast<raw_hash_set*>(this)->find(key, hashval);
    }
    template <class K = key_type>
    LL_INLINE const_iterator find(const key_arg<K>& key) const {
        return find(key, this->hash(key));
    }

    template <class K = key_type>
    LL_INLINE bool contains(const key_arg<K>& key) const {
        return find(key) != end();
    }

    template <class K = key_type>
    LL_INLINE bool contains(const key_arg<K>& key, size_t hashval) const {
    ...
    LL_INLINE friend bool operator!=(const raw_hash_set& a, const raw_hash_set& b) {
        return !(a == b);
    }

    LL_INLINE friend void swap(raw_hash_set& a,
                               raw_hash_set& b) noexcept(noexcept(a.swap(b))) {
        a.swap(b);
    }

    template <class K>
    LL_INLINE size_t hash(const K& key) const {
...
private:
    ...
    {
        template <class K, class... Args>
        LL_INLINE size_t operator()(const K& key, Args&&...) const {
#ifdef PHMAP_NO_MIXING
            return (size_t)h(key);
#else
    ...
    LL_NO_INLINE void drop_deletes_without_resize() {
...
protected:
    ...
    LL_NO_INLINE size_t prepare_insert(size_t hashval) {
        auto target = find_first_non_full(hashval);
...
private:
    ...
    LL_INLINE size_t& growth_left() { return std::get<0>(settings_); }

    LL_INLINE const size_t& growth_left() const { return std::get<0>(settings_); }
...
public:
    ...
    LL_INLINE Inner& get_inner(size_t idx) {
        return  sets_[idx];
    }
    ...
    LL_INLINE void _erase(const_iterator cit) { _erase(cit.iter_); }

    // This overload is necessary because otherwise erase<K>(const K&) would be
    // a better match if non-const iterator is passed as an argument.
    // Do not use erase APIs taking iterators when accessing the map concurrently
    // --------------------------------------------------------------------
    LL_INLINE iterator erase(iterator it) { _erase(it++); return it; }
    ...
    LL_INLINE node_type extract(const_iterator position) {
    ...
    // --------------------------------------------------------------------
    template <class K = key_type>
    LL_INLINE size_t count(const key_arg<K>& key) const {
    ...
    // specific benchmarks indicating its importance.
    // --------------------------------------------------------------------
    LL_INLINE void prefetch_hash(size_t hashval) const {
    ...
    template <class K = key_type>
    LL_INLINE void prefetch(const key_arg<K>& key) const {
        prefetch_hash(this->hash(key));
    }
    ...
    // --------------------------------------------------------------------
    template <class K = key_type>
    LL_INLINE iterator find(const key_arg<K>& key, size_t hashval) {
        SharedLock m;
    ...
    template<class Mtx2_>
    LL_INLINE friend void swap(parallel_hash_set& a,
...
// ----------------------------------------------------------------------------
LL_INLINE std::pair<std::tuple<>, std::tuple<>> PairArgs() { return {}; }
...
struct StringHashEqT
{
    ...
    struct Eq {
        ...
        LL_INLINE bool operator()(std::basic_string_view<CharT> lhs,
...
namespace phmap {
    ...
    LL_INLINE std::size_t erase_if(phmap::flat_hash_set<T, Hash, Eq, Alloc>& c, Pred pred) {
        return phmap::priv::erase_if(c, std::move(pred));
    }

    template <class T, class Hash, class Eq, class Alloc, class Pred>
    LL_INLINE std::size_t erase_if(phmap::node_hash_set<T, Hash, Eq, Alloc>& c, Pred pred) {
        return phmap::priv::erase_if(c, std::move(pred));
    }

    template <class T, class Hash, class Eq, class Alloc, size_t N, class Mtx_, class Pred>
    LL_INLINE std::size_t erase_if(phmap::parallel_flat_hash_set<T, Hash, Eq, Alloc, N, Mtx_>& c, Pred pred) {
        return phmap::priv::erase_if(c, std::move(pred));
    }

    template <class T, class Hash, class Eq, class Alloc, size_t N, class Mtx_, class Pred>
    LL_INLINE std::size_t erase_if(phmap::parallel_node_hash_set<T, Hash, Eq, Alloc, N, Mtx_>& c, Pred pred) {
        return phmap::priv::erase_if(c, std::move(pred));
    }

    // ======== erase_if for phmap map containers ==================================
    template <class K, class V, class Hash, class Eq, class Alloc, class Pred>
    LL_INLINE std::size_t erase_if(phmap::flat_hash_map<K, V, Hash, Eq, Alloc>& c, Pred pred) {
        return phmap::priv::erase_if(c, std::move(pred));
    }

    template <class K, class V, class Hash, class Eq, class Alloc, class Pred>
    LL_INLINE std::size_t erase_if(phmap::node_hash_map<K, V, Hash, Eq, Alloc>& c, Pred pred) {
        return phmap::priv::erase_if(c, std::move(pred));
    }

    template <class K, class V, class Hash, class Eq, class Alloc, size_t N, class Mtx_, class Pred>
    LL_INLINE std::size_t erase_if(phmap::parallel_flat_hash_map<K, V, Hash, Eq, Alloc, N, Mtx_>& c, Pred pred) {
        return phmap::priv::erase_if(c, std::move(pred));
    }

    template <class K, class V, class Hash, class Eq, class Alloc, size_t N, class Mtx_, class Pred>
    LL_INLINE std::size_t erase_if(phmap::parallel_node_hash_map<K, V, Hash, Eq, Alloc, N, Mtx_>& c, Pred pred) {
        return phmap::priv::erase_if(c, std::move(pred));
    }
