...
public:
	...
	virtual bool handleAttach(LLEventDispatcher* dispatcher) override;
	virtual bool handleDetach(LLEventDispatcher* dispatcher) override;
...
public:
	...
	// removed on addtion.
	void addListener(LLEventListener* listener, LLSD filter,
					 const LLSD& userdata);
...
public:
	...
	LL_INLINE void addListener(LLEventListener* listener, LLSD filter = "",
							   const LLSD& userdata = "")
	{
		if (mDispatcher.notNull())
		{
			mDispatcher->addListener(listener, filter, userdata);
		}
	}
	LL_INLINE void removeListener(LLEventListener* listener)
	{
		if (mDispatcher.notNull())
		{
...
{
public:
	LLValueChangedEvent(LLObservable* source, LLSD value)
	:	LLEvent(source, "value_changed"),
		mValue(value)
	{
	}
