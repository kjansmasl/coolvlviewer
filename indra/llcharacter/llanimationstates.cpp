...
LLUUID const ANIM_AGENT_FEMALE_LAND			("ca1baf4d-0a18-5a1f-0330-e4bd1e71f09e");
...
// These used to be in llvoavatar.cpp, but they are better here, so that we can
// add them to the gAnimLibrary in here instead. HB
LLUUID const ANIM_AGENT_BODY_NOISE			("9aa8b0a6-0c6f-9518-c7c3-4f41f2c001ad");
LLUUID const ANIM_AGENT_BREATHE_ROT			("4c5a103e-b830-2f1c-16bc-224aa0ad5bc8");
LLUUID const ANIM_AGENT_EDITING				("2a8eba1d-a7f8-5596-d44a-b4977bf8c8bb");
LLUUID const ANIM_AGENT_EYE					("5c780ea8-1cd1-c463-a128-48c023f6fbea");
LLUUID const ANIM_AGENT_FLY_ADJUST			("db95561f-f1b0-9f9a-7224-b12f71af126e");
LLUUID const ANIM_AGENT_HAND_MOTION			("ce986325-0ba7-6e6e-cc24-b17c4b795578");
LLUUID const ANIM_AGENT_HEAD_ROT			("e6e8d1dd-e643-fff7-b238-c6b4b056a68d");
LLUUID const ANIM_AGENT_PELVIS_FIX			("0c5dd2a2-514d-8893-d44d-05beffad208b");
LLUUID const ANIM_AGENT_TARGET				("0e4896cb-fba4-926c-f355-8720189d5b55");
LLUUID const ANIM_AGENT_WALK_ADJUST			("829bc85b-02fc-ec41-be2e-74cc6dd7215d");
LLUUID const ANIM_AGENT_PHYSICS_MOTION		("7360e029-3cb8-ebc4-863e-212df440d987");
LLUUID const ANIM_AGENT_PUPPET_MOTION		("e6f6a440-b547-11da-a94d-0800200c9a66");


LLUUID AGENT_WALK_ANIMS[] = { ANIM_AGENT_WALK,
							  ANIM_AGENT_RUN,
...
LLUUID AGENT_GUN_HOLD_ANIMS[] = { ANIM_AGENT_HOLD_RIFLE_R,
								  ANIM_AGENT_HOLD_HANDGUN_R,
								  ANIM_AGENT_HOLD_BAZOOKA_R,
								  ANIM_AGENT_HOLD_BOW_L };
S32 NUM_AGENT_GUN_HOLD_ANIMS = LL_ARRAY_SIZE(AGENT_GUN_HOLD_ANIMS);

LLUUID AGENT_GUN_AIM_ANIMS[] = { ANIM_AGENT_AIM_RIFLE_R,
...
:	mAnimStringTable(16384)
{
	...
	// These are added to gAnimLibrary in llvoavatar.cpp in LL's viewer. HB
	mAnimMap[ANIM_AGENT_BODY_NOISE] = mAnimStringTable.addString("body_noise");
	mAnimMap[ANIM_AGENT_BREATHE_ROT] = mAnimStringTable.addString("breathe_rot");
	mAnimMap[ANIM_AGENT_PHYSICS_MOTION] = mAnimStringTable.addString("physics_motion");
	mAnimMap[ANIM_AGENT_EDITING] = mAnimStringTable.addString("editing");
	mAnimMap[ANIM_AGENT_EYE] = mAnimStringTable.addString("eye");
	mAnimMap[ANIM_AGENT_FLY_ADJUST] = mAnimStringTable.addString("fly_adjust");
	mAnimMap[ANIM_AGENT_HAND_MOTION] = mAnimStringTable.addString("hand_motion");
	mAnimMap[ANIM_AGENT_HEAD_ROT] = mAnimStringTable.addString("head_rot");
	mAnimMap[ANIM_AGENT_PELVIS_FIX] = mAnimStringTable.addString("pelvis_fix");
	mAnimMap[ANIM_AGENT_TARGET] = mAnimStringTable.addString("target");
	mAnimMap[ANIM_AGENT_WALK_ADJUST] = mAnimStringTable.addString("walk_adjust");
	mAnimMap[ANIM_AGENT_PUPPET_MOTION] = mAnimStringTable.addString("puppet_motion");
}

// Returns the text name of an animation state
const char* LLAnimationLibrary::animStateToString(const LLUUID& state)
{
	...
	anim_name_map_t::iterator it = mAnimMap.find(state);
	if (it == mAnimMap.end())
	{
		return NULL;
	}
...
// Returns the animation state for a given name
LLUUID LLAnimationLibrary::stringToAnimState(const std::string& name,
											 bool allow_ids)
{
	LLUUID id;
	...
	if (true_name)
	{
		for (anim_name_map_t::iterator iter = mAnimMap.begin();
...
std::string LLAnimationLibrary::animationName(const LLUUID& id)
{
	const char* name = animStateToString(id);
	return name ? std::string(name) : id.asString();
}
