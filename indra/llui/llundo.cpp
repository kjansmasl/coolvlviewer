...
// *TODO: implement doubly linked circular list for ring buffer this will allow
// us to easily change the size of an undo buffer on the fly
...
bool LLUndoBuffer::undoAction()
{
	...
	while(mActions[prevAction]->mClusterID == mOperationID)
	{
		...
		// we're at the first action, so we don't know if we've actually undid
		// everything
		if (mNextAction == mFirstAction)
		{
			--mOperationID;
		...
		// do wrap-around of index, but avoid negative numbers for modulo
		// operator
