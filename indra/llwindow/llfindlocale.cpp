...
static void lang_country_variant_from_envstring(const char* str, char** lang,
												char** country, char** variant)
{
	int end = 0;
	int start;
	...
	if (str[end] && str[end]!=':') // Not at end of st
	{
		++end;
	}
...
constexpr S32 num_primary_to_code = LL_ARRAY_SIZE(primary_to_code);
constexpr S32 num_both_to_code = LL_ARRAY_SIZE(both_to_code);

static S32 lcid_to_fl(LCID lcid, FL_Locale* rtn)
{
	LANGID langid = LANGIDFROMLCID(lcid);
	LANGID primary_lang = PRIMARYLANGID(langid);
	// Try to find an exact primary/sublanguage combo that we know about
	for (S32 i = 0; i < num_both_to_code; ++i)
	{
	...
	for (S32 i = 0; i < num_primary_to_code; ++i)
	{
...
#else	// POSIX
    // Examples: sv_SE.ISO_8859-1, fr_FR.ISO8859-1, no_NO_NB, de_DE
    // Try the various vars in decreasing order of authority
	if (accumulate_env("LC_ALL", rtn) || accumulate_env("LC_MESSAGES", rtn) ||
		accumulate_env("LANG", rtn) || accumulate_env("LANGUAGE", rtn))
	{
		success = FL_CONFIDENT;
	}
	else if (success == FL_FAILED)
