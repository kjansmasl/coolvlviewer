...
void setupCocoa()
{
	...
	if(!inited)
	{
		...
		// The following prevents the Cocoa command line parser from trying
		// to open 'unknown' arguements as documents. I.e. running:
		// './secondlife -set Language fr' would cause a pop-up saying cannot
...
const unsigned short* copyFromPBoard()
{
	...
	NSString* str = NULL;
	BOOL ok =
		[pboard canReadObjectForClasses:classArray options:[NSDictionary dictionary]];
	if (ok)
	{
		NSArray* objToPaste =
			[pboard readObjectsForClasses:classArray options:[NSDictionary dictionary]];
...
// This is currently unused, since we want all our cursors to persist for the
// life of the app, but I've included it for completeness.
...
GLViewRef createOpenGLView(NSWindowRef window, unsigned int samples, bool vsync, bool core_gl)
{
	LLOpenGLView* glview =
		[[LLOpenGLView alloc]initWithFrame:[(LLNSWindow*)window frame] withSamples:samples andVsync:vsync andCoreGL:core_gl];
...
void setWinTitle(NSWindowRef window, const std::string& title)
{
...
{
	LLNSWindow* winRef =
		[(LLAppDelegate*)[[NSApplication sharedApplication] delegate] window];
