...
LL_INLINE F32 get_wrapping_distance(F32 begin, F32 end)
{
	if (begin < end)
	{
		return end - begin;
...
LLSettingsDay::cycle_track_it_t
	get_wrapping_atafter(LLSettingsDay::cycle_track_t& collection, F32 key)
{
	if (collection.empty())
	{
	...
	LLSettingsDay::cycle_track_it_t it = collection.upper_bound(key);
...
LLSettingsDay::cycle_track_it_t
	get_wrapping_atbefore(LLSettingsDay::cycle_track_t& collection, F32 key)
{
	if (collection.empty())
	{
		return collection.end();
	...
	LLSettingsDay::cycle_track_it_t it = collection.lower_bound(key);
	if (it == collection.end())
	{
...
LL_INLINE LLSettingsDay::track_bound_t
	get_bounding_entries(LLSettingsDay::cycle_track_t& track, F32 keyframe)
{
	return LLSettingsDay::track_bound_t(get_wrapping_atbefore(track, keyframe),
										get_wrapping_atafter(track, keyframe));
}

// Find normalized track position of given time along full length of cycle
LL_INLINE F32 convert_time_to_position(F64 time, F64 len)
{
	return time > 0.0 && len > 0.0 ? F32(fmod(time, len) / len)
								   : (time <= 0.0 ? 0.f : 1.f);
}

LL_INLINE F64 convert_time_to_blend_factor(F64 time, F64 len,
										   LLSettingsDay::cycle_track_t& track)
{
	F32 position = convert_time_to_position(time, len);
	LLSettingsDay::track_bound_t bounds = get_bounding_entries(track,
															   position);
...
// LLEnvironmentRequest (seen in llwlhanders.h/cpp in LL's viewer).
...
public:
	...
	static bool initiate(LLEnvironment::env_apply_fn cb);

private:
	static void onRegionCapsReceived(const LLUUID& region_id,
									 LLEnvironment::env_apply_fn cb);
	static bool doRequest(LLEnvironment::env_apply_fn cb);

 private:
	static void environmentRequestCoro(const std::string& url,
									   LLEnvironment::env_apply_fn cb);
...
bool LLEnvironmentRequest::initiate(LLEnvironment::env_apply_fn cb)
{
	...
	if (!regionp->capabilitiesReceived())
	{
		llinfos << "Deferring environment settings request until capabilities are received."
				<< llendl;
		regionp->setCapsReceivedCB(boost::bind(&LLEnvironmentRequest::onRegionCapsReceived,
											   _1, cb));
		return false;
	}
...
public:
		...
		mCycleOffset(cycleoffset)
	{
		...
		LLSettingsDay::track_bound_t initial = getBoundingEntries(now);
	...
	void switchTrack(S32 trackno, F32)
	{
		...
		LLSettingsDay::track_bound_t bounds = getBoundingEntries(now);
...
protected:
	S32 selectTrackNumber(S32 trackno)
	{
		...
		for (S32 test = trackno; test != 0; --test)
		{
			...
			LLSettingsDay::cycle_track_t& track = mDay->getCycleTrack(test);
			if (!track.empty())
			{
				return test;
			}
	...
	LLSettingsDay::track_bound_t getBoundingEntries(F64 time)
	{
		LLSettingsDay::cycle_track_t& wtrack = mDay->getCycleTrack(mTrackNo);
	...
	LL_INLINE F64 getAdjustedNow() const
	{
		return LLTimer::getEpochSeconds() + mCycleOffset;
	}

	F64 getSpanTime(const LLSettingsDay::track_bound_t &bounds) const
	{
...
private:
	void onFinishedSpan()
	{
		...
		LLSettingsDay::track_bound_t next = getBoundingEntries(adjusted_now);
...
public:
	...
	LL_INLINE typename SETTINGT::ptr_t getSource() const
	{
		return mSource;
	}
	...
	LL_INLINE bool isDirty() const override
	{
		return SETTINGT::isDirty() || mSource->isDirty();
	}

	LL_INLINE bool isVeryDirty() const override
	{
	...
	LL_INLINE bool hasInjections() const
	{
...
private:
	LL_INLINE static bool compare_remaining(const typename Injection::ptr_t& a,
											const typename Injection::ptr_t& b)
	{
		return a->mTimeRemaining < b->mTimeRemaining;
	}

	LL_INLINE static bool has_remaining(const typename Injection::ptr_t& a)
	{
...
public:
	...
	LL_INLINE void animate() override				{}

	LL_INLINE LLEnvironment::DayInstance::ptr_t getBaseDayInstance() const
	{
	...
	LL_INLINE S32 countExperiencesActive() const	{ return mActiveExperiences.size(); }

	LL_INLINE bool isOverriddenSky() const			{ return mSkyExperience.notNull(); }
	LL_INLINE bool isOverriddenWater() const		{ return mWaterExperience.notNull(); }
...
{
	if (!mInjection || !mNextInstance)
	{
		llwarns << "NULL pointer detected. Operation aborted" << llendl;
		return;
	}

	mNextInstance->animate();

	if (!mInjection->isOverriddenSky() && mStartSky)
	{
	...
	if (mInjection->isOverriddenWater() || !mStartWater)
	{
...
const LLUUID LLEnvironment::KNOWN_SKY_MIDDAY_PBR("d9950f9a-f693-be73-0384-13ee97b8ca10");
...
bool LLEnvironment::canAgentUpdateParcelEnvironment(LLParcel* parcel)
{
	...
	if (!parcel || !gAgent.hasExtendedEnvironment())
	{
		return false;
	}
...
void LLEnvironment::setFixedTimeOfDay(F32 position, EEnvSelection env,
									  S32 env_version)
{
	...
	DayInstance::ptr_t envp = getEnvironmentInstance(env, true);
	LLSettingsDay::ptr_t dayp = envp->getDayCycle();
#if 0	// *TODO: get the parcel environment fully cloned as a new day cycle.
	if (!dayp && env == ENV_LOCAL)
	{
		// Try and use the parcel day cycle instead
		DayInstance::ptr_t parcelp = getEnvironmentInstance(ENV_PARCEL, true);
		if (parcelp)
		{
			dayp = parcelp->getDayCycle();
		}
	}
#endif
	if (!dayp)
	{
		// Problem: loading the default day sycle also loads the default
		// Windlight sky and water settings...
	...
	setEnvironment(env, skyp, waterp, env_version);
								   ...
								   S32 env_version)
{
	...
	if (reset_probes && gUsePBRShaders)
	{
															  ...
															  bool resolve)
{
	if (env == ENV_CURRENT || resolve)
	{
			 ...
			 idx < (U32)ENV_END; ++idx)
		{
			...
			DayInstance::ptr_t dayp =
				getEnvironmentInstance((EEnvSelection)idx);
			if (dayp)
			{
				if (!fixed.first)
				{
					fixed.first = dayp->getSky();
				}
				if (!fixed.second)
				{
					fixed.second = dayp->getWater();
				}
			}
		}
	...
	if (env >= ENV_EDIT && env <= ENV_DEFAULT)
	{
		DayInstance::ptr_t dayp = getEnvironmentInstance(env);
		if (dayp)
		{
			return fixed_env_t(dayp->getSky(), dayp->getWater());
		}
	}
	else
	{
...
void LLEnvironment::update()
{
	...
	// Make sure the current environment does not go away until
	// applyTimeDelta() is done.
	DayInstance::ptr_t keeper = mCurrentEnvironment;

	// Detect changes happening in consecutive frames, and pause refelection
	// probes update when they happen, resuming the latter when they are over.
	static bool last_changed = false;
	bool env_changed = mCurrentEnvironment->applyTimeDelta(delta);
	if (env_changed && last_changed)
	{
		// Pause reflection probes updates during environment changes. HB
		gPipeline.mReflectionMapManager.pause(true);
	}
	else if (!env_changed && !last_changed)
	{
		// Resume reflection probes updates after changes are over. HB
		gPipeline.mReflectionMapManager.pause(false);
	}
	last_changed = env_changed;
	...
	// updates later...
	mLastCamYaw = gViewerCamera.getYaw() + SUN_DELTA_YAW;

	// Cache the environment values for reuse by the render pipeline during the
	// upcomming frame. HB
	gPipeline.cacheEnvironment();
												 ...
												 const LLSettingsBase::ptr_t& settings)
{
	...
	LLShaderUniforms* shaderp = &uniforms[LLGLSLShader::SG_ANY];
	...
	for (const auto& it : params)
	{
		...
		switch (setting_type)
		{
			case LLSD::TypeInteger:
				shaderp->uniform1i(shader_key, value.asInteger());
				break;

			case LLSD::TypeReal:
				shaderp->uniform1f(shader_key, value.asReal());
				break;

			case LLSD::TypeBoolean:
				shaderp->uniform1i(shader_key, value.asBoolean() ? 1 : 0);
				break;
			...
			case LLSD::TypeArray:
				if (gUsePBRShaders)
				{
					...
					{
						// Maximize and remove tinting if this is an irradiance
					...
					shaderp->uniform3fv(shader_key, LLVector3(vect4.mV));
				}
				else
				{
					shaderp->uniform4fv(shader_key, LLVector4(value));
				}
...
void LLEnvironment::updateShaderUniforms(LLGLSLShader* shaderp)
{
	// Apply uniforms that should be applied to all shaders
	mSkyUniforms[LLGLSLShader::SG_ANY].apply(shaderp);
	mWaterUniforms[LLGLSLShader::SG_ANY].apply(shaderp);

	// Apply uniforms specific to the given shader's shader group
	S32 group = shaderp->mShaderGroup;
	mSkyUniforms[group].apply(shaderp);
	mWaterUniforms[group].apply(shaderp);
}

void LLEnvironment::updateShaderSkyUniforms(LLGLSLShader* shaderp)
{
	// Apply uniforms that should be applied to all shaders
	mSkyUniforms[LLGLSLShader::SG_ANY].apply(shaderp);
...
void LLEnvironment::dirtyUniforms()
{
	...
	// Now scan the list of shaders that might be interested in environment
	// updates, and mark them as having dirty uniforms.
	const LLViewerShaderMgr::shaders_list_t& shaders =
		gViewerShaderMgrp->getEnvShadersList();
	for (U32 i = 0, count = shaders.size(); i < count; ++i)
	{
		LLGLSLShader* shaderp = shaders[i];
		if (shaderp->mProgramObject &&
			(can_use_shaders ||
			 shaderp->mShaderGroup == LLGLSLShader::SG_WATER))
		{
			shaderp->mUniformsDirty = true;
		}
	}
}
											 ...
											 LLSD data, F32 transition_time)
{
	...
	{
		environment->injectWaterSettings(water, experience_id,
										 F64(transition_time));
	}
...
bool LLEnvironment::DayInstance::setSky(const LLSettingsSky::ptr_t& skyp)
{
	mType = TYPE_FIXED;
	mInitialized = false;

	bool changed = !skyp || !mSky || skyp->getHash() != mSky->getHash();
...
void LLEnvironment::DayInstance::animate()
{
	...
	{
		LLSettingsDay::cycle_track_t& wtrack = mDayCycle->getCycleTrack(0);
		if (wtrack.empty())
		{
			mWater.reset();
			mBlenderWater.reset();
	...
	if (!(mAnimateFlags & NO_ANIMATE_SKY))
	{
		...
		LLSettingsDay::cycle_track_t &track = mDayCycle->getCycleTrack(1);
		if (track.empty())
		{
			mSky.reset();
			mBlenderSky.reset();
...
{
	if (!mNextInstance || !mStartWater || !mStartSky)
	{
		llwarns << "NULL pointer detected. Operation aborted" << llendl;
		return;
	}
...
:	LLSettingsBlender(target, LLSettingsBase::ptr_t(),
	...
	mPosition(0.0)
{
	LLSettingsDay::track_bound_t initial = getBoundingEntries(mPosition);
...
F64 LLTrackBlenderLoopingManual::setPosition(F32 position)
{
	...
	LLSettingsDay::track_bound_t bounds = getBoundingEntries(mPosition);
...
LLSettingsDay::track_bound_t LLTrackBlenderLoopingManual::getBoundingEntries(F64 position)
{
	LLSettingsDay::cycle_track_t& wtrack = mDay->getCycleTrack(mTrackNo);
...
F64 LLTrackBlenderLoopingManual::getSpanLength(const LLSettingsDay::track_bound_t& bounds) const
{
