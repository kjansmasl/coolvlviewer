...
public:
	...
	// Gets the total LLSD
	LL_INLINE const LLSD& getAll() const		{ return mParamValues; }
...
struct WaterColorControl
{
	...
	LL_INLINE WaterColorControl(F32 red, F32 green, F32 blue, F32 alpha,
	...
	LL_INLINE WaterColorControl& operator=(const LLColor4& val)
	{
		mR = val.mV[0];
		mG = val.mV[1];
		mB = val.mV[2];
	...
	LL_INLINE operator LLColor4(void) const
	{
		return LLColor4(mR, mG, mB, mA);
	}

	LL_INLINE WaterColorControl& operator=(const LLVector4& val)
	{
		mR = val.mV[0];
		mG = val.mV[1];
		mB = val.mV[2];
	...
	LL_INLINE operator LLVector4(void) const
	{
		return LLVector4(mR, mG, mB, mA);
	}

	LL_INLINE operator LLVector3(void) const
	{
		return LLVector3(mR, mG, mB);
	}

	LL_INLINE void update(LLWaterParamSet& params) const
	{
...
struct WaterVector3Control
{
	...
	// Basic constructor
	LL_INLINE WaterVector3Control(F32 valX, F32 valY, F32 valZ,
								  const std::string& n)
	:	mX(valX),
		mY(valY),
		mZ(valZ),
	...
	LL_INLINE WaterVector3Control& operator=(const LLVector3& val)
	{
		mX = val.mV[0];
		mY = val.mV[1];
		mZ = val.mV[2];
...
struct WaterVector2Control
{
	...
	// basic constructor
	LL_INLINE WaterVector2Control(F32 valX, F32 valY, const std::string& n)
	:	mX(valX),
		mY(valY),
		mName(n)
	{
	...
	LL_INLINE WaterVector2Control& operator=(const LLVector2& val)
	{
		mX = val.mV[0];
		mY = val.mV[1];
		return *this;
...
struct WaterFloatControl
{
	...
	LL_INLINE WaterFloatControl(F32 val, const std::string& n, F32 m = 1.f)
	:	mX(val),
		mName(n),
		mMult(m)
	{
	...
	LL_INLINE WaterFloatControl& operator = (const LLVector4& val)
	{
		mX = val.mV[0];
		return *this;
	}

	LL_INLINE operator F32(void) const
	{
		return mX;
	}
...
struct WaterExpFloatControl
{
	...
	LL_INLINE WaterExpFloatControl(F32 val, const std::string& n, F32 b)
	:	mExp(val),
		mName(n),
		mBase(b)
	{
	...
	LL_INLINE WaterExpFloatControl& operator=(F32 val)
	{
...
public:
	...
	LL_INLINE bool setParamSet(const std::string& name, LLWaterParamSet& param)
	{
