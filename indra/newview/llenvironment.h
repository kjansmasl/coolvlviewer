...
public:
	...
	// Using an enum here instead of static const or constexpr variables makes
	// it easier should we add more values later. HB
	enum EETransition
	{
	...
	LL_INLINE bool canEdit() const					{ return true; }
	...
	void setFixedTimeOfDay(F32 position, EEnvSelection env = ENV_LOCAL,
						   S32 env_version = NO_VERSION);

	void setLocalEnvFromDefaultWindlightDay(F32 position = -1.f);

	LL_INLINE LLSettingsDay::ptr_t getCurrentDay() const
	{
	...
	LL_INLINE EEnvSelection getSelectedEnvironment() const
	{
		return mSelectedEnvironment;
	}

	// Apply current settings to given shader
	void updateShaderUniforms(LLGLSLShader* shaderp);
	// Apply current sky settings to given shader
	void updateShaderSkyUniforms(LLGLSLShader* shaderp);

	// Prepare settings to be applied to shaders (call whenever settings are
	// updated)
	void updateSettingsUniforms();
	// Flags uniforms for shaders using environment settings as dirty. HB
	void dirtyUniforms();
	...
	LL_INLINE void setEnvironment(EEnvSelection env,
	...
	LL_INLINE LLSettingsSky::ptr_t getEnvironmentFixedSky(EEnvSelection env,
														  bool resolve = false)
	{
		return getEnvironmentFixed(env, resolve).first;
	}

	LL_INLINE LLSettingsWater::ptr_t getEnvironmentFixedWater(EEnvSelection env,
															  bool resolve = false)
	{
	...
	LL_INLINE LLVector2 getCloudScrollDelta() const	{ return mCloudScrollDelta; }

	LL_INLINE void pauseCloudScroll()				{ mCloudScrollPaused = true; }
	LL_INLINE void resumeCloudScroll()				{ mCloudScrollPaused = false; }
	LL_INLINE bool isCloudScrollPaused() const		{ return mCloudScrollPaused; }

	F32 getCamHeight() const;
	bool getIsSunUp() const;
	bool getIsMoonUp() const;

	LL_INLINE static void addBeaconsUser()			{ ++sBeaconsUsers; }
	...
	LL_INLINE LLVector4 getLightDirectionCFR() const
	{
		return toCFR(getLightDirection());
	}

	LL_INLINE LLVector4 getSunDirectionCFR() const
	{
		return toCFR(getSunDirection());
	}

	LL_INLINE LLVector4 getMoonDirectionCFR() const
	{
	...
	LL_INLINE F32 getProgress() const
	{
		return mCurrentEnvironment ? mCurrentEnvironment->getProgress() : -1.f;
	}

	LL_INLINE F32 getRegionProgress() const
	{
	...
	LL_INLINE connection_t setEnvironmentChanged(env_changed_fn cb)
	{
	...
	LL_INLINE const altitude_list_t& getRegionAltitudes() const
	{
	...
	public:
		...
		virtual void setDay(const LLSettingsDay::ptr_t& dayp, S32 daylength,
							S32 dayoffset);
		virtual bool setSky(const LLSettingsSky::ptr_t& skyp);
		...
		const LL_INLINE LLSettingsDay::ptr_t& getDayCycle() const
		{
			return mDayCycle;
		}

		const LL_INLINE LLSettingsSky::ptr_t& getSky() const
		{
			return mSky;
		}

		const LL_INLINE LLSettingsWater::ptr_t& getWater() const
		{
			return mWater;
		}

		LL_INLINE S32 getDayLength() const			{ return mDayLength; }
		LL_INLINE S32 getDayOffset() const			{ return mDayOffset; }
		LL_INLINE S32 getSkyTrack() const			{ return mSkyTrack; }

		LL_INLINE void setDayOffset(F64 offset)
		{
			mDayOffset = offset;
			animate();
		}
		...
		LL_INLINE EEnvSelection getEnvironmentSelection() const
		{
			return mEnv;
		}

		LL_INLINE void setEnvironmentSelection(EEnvSelection env)
		{
			mEnv = env;
		}
		...
		LL_INLINE void setFlags(U32 flag)			{ mAnimateFlags |= flag; }
		LL_INLINE void clearFlags(U32 flag)			{ mAnimateFlags &= ~flag; }
...
private:
	LL_INLINE static LLVector4 toCFR(const LLVector3& vec)
	{
		return LLVector4(vec.mV[1], vec.mV[0], vec.mV[2], 0.f);
	}

	LL_INLINE static LLVector4 toLightNorm(const LLVector3& vec)
	{
...
public:
    ...
    void switchTrack(S32 trackno, F32 position) override;

	LL_INLINE S32 getTrack() const					{ return mTrackNo; }

protected:
	LLSettingsDay::track_bound_t getBoundingEntries(F64 position);
	F64 getSpanLength(const LLSettingsDay::track_bound_t& bounds) const;
