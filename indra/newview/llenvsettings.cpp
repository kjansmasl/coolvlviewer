...
void LLEnvSettingsSky::applySpecial(LLShaderUniforms* targetp)
{
	...
	if (shaderp)
	{
		...
		if (gUsePBRShaders)
		{
			...
			shaderp->uniform3fv(LLShaderMgr::SUNLIGHT_COLOR,
								LLColor3(sun_diffuse).mV);
			shaderp->uniform3fv(LLShaderMgr::MOONLIGHT_COLOR,
								LLColor3(moon_diffuse).mV);
		...
		else
		{
			shaderp->uniform4fv(LLShaderMgr::CLOUD_POS_DENSITY1,
								vect_c_p_d1.mV);
			shaderp->uniform4fv(LLShaderMgr::SUNLIGHT_COLOR, sun_diffuse.mV);
			shaderp->uniform4fv(LLShaderMgr::MOONLIGHT_COLOR, moon_diffuse.mV);
	...
	if (gUsePBRShaders)
	{
		...
		static LLCachedControl<F32> sky_scale(gSavedSettings,
											  "RenderSkyAmbientScale");
		shaderp->uniform1f(LLShaderMgr::SKY_SUNLIGHT_SCALE, sun_scale);
		shaderp->uniform1f(LLShaderMgr::SKY_AMBIENT_SCALE, sky_scale);
		...
		{
			const LLSettingsSky::ptr_t& skyp = gEnvironment.getCurrentSky();
			if (skyp->getReflectionProbeAmbiance() != 0.f)
			{
			...
			else if (adjust && skyp->canAutoAdjust())
			{
				hdr_scale = hdr_adjust;
				// Auto-adjust legacy sky to take advantage of probe ambiance.
				ambient = getTotalAmbient() * F32(amb_scale);

				LLColor3 sun_diffuse = getSunDiffuse() * col_scale;
				shaderp->uniform3fv(LLShaderMgr::SUNLIGHT_COLOR, sun_diffuse.mV);

				LLColor3 blue_density = getBlueDensity() * den_scale;
				shaderp->uniform3fv(LLShaderMgr::BLUE_DENSITY, blue_density.mV);

				LLColor3 blue_horizon = getBlueHorizon() * hor_scale;
				shaderp->uniform3fv(LLShaderMgr::BLUE_HORIZON, blue_horizon.mV);			

				// Note: this must match the value returned when auto_adjust
				// is true by LLSettingsSky::getReflectionProbeAmbiance()
				probe_ambiance = probe_amb;
				LLSettingsSky::sAutoAdjustProbeAmbiance = probe_ambiance;
			}
			else
			{
				hdr_scale = 1.f;
	...
	shaderp->uniform1i(LLShaderMgr::SUN_UP_FACTOR, getIsSunUp() ? 1 : 0);
	shaderp->uniform1f(LLShaderMgr::SUN_MOON_GLOW_FACTOR,
					   getSunMoonGlowFactor());
	shaderp->uniform1f(LLShaderMgr::DENSITY_MULTIPLIER, getDensityMultiplier());
...
void LLEnvSettingsWater::applySpecial(LLShaderUniforms* targetp)
{
	...
	if (gUsePBRShaders)
	{
		LLColor4 fog_color(waterp->getWaterFogColor());
