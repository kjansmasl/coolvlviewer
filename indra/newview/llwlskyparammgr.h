...
public:
	...
	LL_INLINE F32 getStarBrightness()				{ return mStartBrightness; }

	void setSunAngle(F32 val);

	LL_INLINE F32 getSunAngle()						{ return mSunAngle; }

	void setEastAngle(F32 val);

	LL_INLINE F32 getEastAngle()					{ return mEastAngle; }

	// Sets the cloud scroll x enable value
	void setEnableCloudScrollX(bool val);

	// Gets the scroll x enable value;
	LL_INLINE bool getEnableCloudScrollX()			{ return mCloudScrollEnableX; }

	// Sets the cloud scroll y enable value
	void setEnableCloudScrollY(bool val);

	// Gets the scroll enable y value
	LL_INLINE bool getEnableCloudScrollY()			{ return mCloudScrollEnableY; }

	// Sets the scroll x value
	void setCloudScrollX(F32 val);

	// Gets the scroll x enable value
	LL_INLINE F32 getCloudScrollX()					{ return mCloudScrollRateX; }

	// Sets the scroll y enable value
	void setCloudScrollY(F32 val);

	// Gets the scroll enable y value
	LL_INLINE F32 getCloudScrollY()					{ return mCloudScrollRateY; }
...
struct WLColorControl
{
	...
	LL_INLINE WLColorControl(F32 red, F32 green, F32 blue, F32 intensity,
	...
	LL_INLINE WLColorControl& operator=(const LLVector4& val)
	{
		r = val.mV[0];
		g = val.mV[1];
		b = val.mV[2];
	...
	LL_INLINE operator LLVector4() const
	{
		return LLVector4(r, g, b, i);
	}

	LL_INLINE operator LLVector3() const
	{
		return LLVector3(r, g, b);
	}

	LL_INLINE void update(LLWLParamSet& params) const
	{
...
struct WLFloatControl
{
	...
	LL_INLINE WLFloatControl(F32 val, const std::string& n, F32 m = 1.f)
	:	x(val),
		mName(n),
		mult(m)
	{
	...
	LL_INLINE WLFloatControl & operator=(const LLVector4& val)
	{
		x = val.mV[0];
		return *this;
	}

	LL_INLINE operator F32 () const
	{
		return x;
	}
...
public:
	...
	LL_INLINE void setTrack(const std::map<F32, std::string>& track,
...
public:
	...
	LL_INLINE LLVector4 getLightDir() const			{ return mLightDir; }

	// Gets where the light is pointing
	LL_INLINE LLVector4 getClampedLightDir() const	{ return mClampedLightDir; }
	...
	LL_INLINE bool setParamSet(const std::string& name, LLWLParamSet& param)
	{
	...
	LL_INLINE void setDirty()						{ mCurrentParamsDirty = true; }
