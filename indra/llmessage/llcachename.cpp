...
// We track name requests in flight for up to this long. We won't re-request a
...
bool LLCacheName::Impl::getName(const LLUUID& id,
...
void LLCacheName::localizeCacheName(std::string key, std::string value)
{
	...
	{
		llwarns << " Error localizing cache key " << key << " To "<< value
				<< llendl;
	}
}
...
void LLCacheName::dump()
{
		 ...
		 iter != end; iter++)
	{
		...
		if (entry->mIsGroup)
		{
			llinfos << iter->first << " = (group) " << entry->mGroupName
					<< " @ " << entry->mCreateTime << llendl;
		}
		else
		{
			llinfos << iter->first << " = "
...
void LLCacheName::Impl::processPendingReplies()
{
	...
	for (it = reply_queue_begin; it != reply_queue_end; ++it)
	{
		...
		if (!entry->mIsGroup)
		{
			std::string fullname = LLCacheName::buildFullName(entry->mFirstName,
															  entry->mLastName);
...
void LLCacheName::Impl::processUUIDRequest(LLMessageSystem* msg, bool isGroup)
{
	...
	for (S32 i = 0; i < count; ++i)
	{
		...
		if (entry)
		{
			...
			{
				llwarns << "Asked for " << (isGroup ? "group" : "user")
						<< " name, but found "
						<< (entry->mIsGroup ? "group" : "user") << ": " << id
						<< llendl;
			}
			else
			{
	...
	for (S32 i = 0; i < count; ++i)
	{
		...
		{	// is group
			msg->getStringFast(_PREHASH_UUIDNameBlock, _PREHASH_GroupName,
							   entry->mGroupName, i);
			LLStringFn::replace_ascii_controlchars(entry->mGroupName,
												   LL_UNKNOWN_CHAR);
		}
		...
		if (!isGroup)
		{
			...
			{
				full_name = LLCacheName::buildFullName(entry->mFirstName,
													   entry->mLastName);
			}
