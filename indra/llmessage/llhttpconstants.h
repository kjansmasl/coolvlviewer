...
LL_INLINE bool isHttpInformationalStatus(S32 status)
{
	// Check for status 1xx.
	return status >= 100 && status < 200;
}

LL_INLINE bool isHttpGoodStatus(S32 status)
{
	// Check for status 2xx.
	return status >= 200 && status < 300;
}

LL_INLINE bool isHttpRedirectStatus(S32 status)
{
	// Check for status 3xx.
	return status >= 300 && status < 400;
}

LL_INLINE bool isHttpClientErrorStatus(S32 status)
{
...
LL_INLINE bool isHttpServerErrorStatus(S32 status)
{
